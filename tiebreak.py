...
H2H_ENABLED = {
    ...
    "QAT": False,   # [مرجح خلاف ذلك] — بحث 5 سبتمبر: نص ويكيبيديا
                    ...
                    # مفتوح 16 بالـREADME.
    "MAR": False,   # [مؤكد لكن الكود لا يكفي] — بحث 5 سبتمبر: نص
                    # ويكيبيديا صريح ومتكرر حرفياً (موسمَي 2023-24
                    # و2025-26): "1) Points؛ 2a) H2H goal difference
                    # **لو فريقان بالضبط** متساويان بالنقاط؛ 2b)
                    # الفارق الكلي **لو أكثر من فريقين**؛ 3) عدد
                    # الفوز؛ 4) الأهداف؛ 5) ملحق". ⚠️ **`sort_table()`
                    # هنا لا يفرّق حجم المجموعة** — يطبّق H2H على أي
                    # عدد فرق متساوية (2 أو أكثر)، فتفعيله كـTrue
                    # يصحّح حالة الفريقين ويكسر حالة 3+ فرق (يطبّق
                    # H2H بدل التخطي المطلوب للفارق الكلي مباشرة).
                    # False الحالي عكسها بالضبط: صحيح لـ3+ فرق، خطأ
                    # لفريقين بالضبط. **لا قيمة واحدة تُصحّح
                    # الحالتين معاً — يحتاج كود جديد يفحص
                    # `len(group)==2` تحديداً، لا مجرد تفعيل/تعطيل.**
                    # قرار مؤجَّل لجلسة منفصلة. راجع بند مفتوح 16.
}
