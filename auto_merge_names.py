...
def main():
    ...
    for f in sorted(dupes, key=lambda x: -max(
        ...
        # ⚠️ 7 سبتمبر — `keep` يُنسَخ حرفياً من قيمة خام بالديتابيس
        #    بلا أي فحص شكل. `fold()` يوحّد المسافات **للتجميع فقط**
        #    (تحديد أن اسمين لنفس اللاعب) — لا يُطبَّق أبداً على القيمة
        #    المكتوبة فعلياً. لو كانت القيمة الأكثر تكراراً نفسها
        #    تالفة (مسافة مزدوجة/طرفية)، كانت تُكتب تالفة بـkeep_name
        #    ثم تُطبَّق فعلياً على الديتابيس بـ`apply_player_merges.py`.
        #    `split()` بلا وسيط يزيل الطرفية والمزدوجة معاً بضربة واحدة.
        #    هذه الثغرة بالذات أنتجت السطر التالف "Mohammed  Abu
        #    Al-Shamat" بـ`player_merges.csv`، واستدعى استرجاعاً كاملاً
        #    لـ`football.db` من نسخة احتياطية (7 سبتمبر) بعد أن كُتب
        #    التلف فعلياً على 80 صفاً قبل اكتشاف السبب هنا.
        cleaned = " ".join(keep.split())
        if cleaned != keep:
            print(f"  ⚠️ تنظيف مسافات بالاسم المختار: {keep!r} → {cleaned!r}")
            keep = cleaned
