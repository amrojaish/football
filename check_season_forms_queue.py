...
#   قرارات 21 سبتمبر 2026 (بند 49): استبعاد نهائي — لا تُكتب أبداً.
#     343983 و534077 و320288: اتجاه معكوس، الترجمة على الجديدة فقط والاسم
#       الأول مختلف (Alaa/Ahmad، Saif/Saud) — كانت ستضع اسماً على شخص آخر.
#     402634: استبعاد نهائي ببند 27 (14 سبتمبر)، صفر دليل نشاط.
DECIDED_EXCLUDED |= {343983, 534077, 320288, 402634}
#   معلَّقة: تصادم غير محسوم (فحص إضافي معقَّد أو لا معرفة شخصية كافية).
DECIDED_PENDING = {440196, 631657, 17087}
#   قرارات 21 سبتمبر 2026 (بند 49): معلَّقة بأسبابها (لا تُكتب الآن).
PENDING_BY_REASON = {
    ...
    "خرق متبادل مستبعَدة من الكتابة: دليل ضعيف (135059 دليل 4/0، 465786 دليل 2/0)، "
    "الترجمة القائمة لا تطابق الاسم الأول (449662)، اسم مفرد شائع (30795)، "
    "صيغة جديدة باسم مختلف ودليل 1/0 (104666)، صفر دليل (2274، 561378)": {
        135059, 465786, 449662, 30795, 104666, 2274, 561378},
}
...
#   41 حالة واضحة (39 معرّفاً) + 8 من فحص فروق التهجئة (534072، 433596، 639230،
#   452299، 286352، 542799، 642884، 44622). ⚠️ تحفّظ صريح: 642884 (تصادم غير
#   محسوم بـ534082 وناديان مختلفان) و542799 (تصادم بـ110747 وناديان مختلفان).
#   423731 صفّان (واضح + تعادل) لكنه بلا عربي أصلاً فيُنقل بمعرّفه كاملاً.
#   ⚠️ تحفّظ على 110747 و452659 أيضاً: تصادم معرّفين (`A. Mahajna` بين 110747 و542799؛
#   `Y. Marei` تصادم 2) — تُحسَم الهوية قبل أي ترجمة.
GENERAL_TRANSLATION = {
    582, 2824, 17150, 21140, 22233, 42036, 42147, 50505, 57169, 96824, 134828,
    162167, 162186, 196773, 199297, 264493, 271349, 283174, 283330, 283564,
    294367, 295249, 306726, 339794, 354063, 366516, 384433, 393210, 394601,
    396630, 408562, 423731, 448925, 498005, 533865, 534073, 550437, 625380, 635415,
    534072, 433596, 639230, 452299, 286352, 542799, 642884, 44622,
    # +6 (بند 49، تحديث لاحق): 5 من الخرق المتبادل بلا عربي + 452659 (فرق تهجئة بلا عربي).
    110747, 194908, 306853, 342418, 368825, 452659,
}
...
def main():
    ...
    print(f"    − {sc['general_translation']} {STATUS_LABEL['general_translation']} (قرار موثَّق، بند 49)")
