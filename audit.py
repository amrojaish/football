#!/usr/bin/env python3
"""
فحص جودة الداتا — تغطية الأهداف المسجَّلة
==========================================
بيقارن النتيجة النهائية (من جدول matches) مع عدد الأهداف
المسجّلة تفصيلياً (من جدول goals) — ويكشف الفجوة.

⚠️ **هذا قياس `goals` لا `events`.** التغطية العالية هنا لا
   تعني أن الدوري له بطاقات أو تشكيلات أو إحصائيات لاعبين —
   تلك طبقة `events` وهي **للسعودي وحده** (درس 4).
...
def main():
    ...
    print("\n" + "#" * 58)
    print("#  فحص جودة الداتا — تغطية الأهداف المسجَّلة")
    print("#" * 58)
